...
@responses.activate
def test_delete_logs_v2_posts_backend_body_and_parses_affected_rows():
    """DeleteLogsV2 posts to /deletelogs and parses affected_rows."""
    client = make_client(endpoint="cn-mock.example.com", project="mock-proj")
    ...
    resp = client.delete_logs_v2(req)
...
@responses.activate
def test_update_logs_posts_backend_body_and_parses_affected_rows():
    """UpdateLogs posts to /updatelogs and leaves data as a string."""
    client = make_client(endpoint="cn-mock.example.com", project="mock-proj")
    ...
    assert resp.get_affected_rows() == 2
...
def test_update_logs_error_response_raises_logexception():
    ...
    mock_sls_response(
        ...
        re.compile(r"https?://mock-proj\.cn-mock\.example\.com.*?/logstores/store-1/updatelogs$"),
        status=400,
        body=error_response("ParameterInvalid", "bad update request"),
    )
